...
float Kd = 90;  // Derivační konstanta (tlumí cukání)
int Max_speed = 100;

int sensorPins[] = {U1, U2, U3, U4, U5, U6, U7};

int minValues[7], maxValues[7];
int threshold[7] = {700, 700, 700, 700, 700, 700, 700};
...
void linefollow() {
  isOn2 = false;
  int error = (50 * IR1 + 10 * IR2 + 5 * IR3 - 5 * IR5 - 10 * IR6 - 50 * IR7);
  ...
  int Lspeed = Max_speed + adjustment;
  int Rspeed = Max_speed - adjustment;
...
void calibrate()
{
  ...
  for (int i = 0; i < 3000; i++) {
    ...
    delay(1); // Small delay to allow the robot to actually move
  }

  for (int i = 0; i < 7; i++) {
    threshold[i] = 7 * (minValues[i] + maxValues[i]) / 12;
  }
